(async function() {
    ...
    function initProfileLogic(data) {
        ...
        document.getElementById('edit-profile-btn').onclick = () => {
            document.getElementById('edit-fullname').value = document.getElementById('info-fullname').textContent;
            sb.classList.add('is-editing');
        };
        ...
        document.getElementById('save-profile-btn').onclick = async () => {
            ...
            document.getElementById('info-fullname').textContent = name;
            document.getElementById('username-display').textContent = name;
            sb.classList.remove('is-editing');
        };
    }
    ...
    function initInterviewListLogic() {
        ...
        listEl.addEventListener('click', (e) => {
            ...
            if(btn.classList.contains('toggle-desc')) {
                const d = btn.closest('li').querySelector('.interview-desc-content');
            ...
            else if(btn.classList.contains('del-int')) {
                if(confirm('Xóa?')) fetch(`${NGROK_BASE_URL}/deleteInterview.php`, { method:'POST', credentials:'include', headers:{'Content-Type':'application/x-www-form-urlencoded','ngrok-skip-browser-warning':'true'}, body:new URLSearchParams({'interview_name':id}) }).then(load);
            }
        });
        document.getElementById('create-interview-form').addEventListener('submit', async (e)=>{
            e.preventDefault();
            const name = document.getElementById('interview-fullname').value;
            ...
            document.getElementById('create-status-msg').textContent = 'Xong!';
            load();
        });
        load();
    }
    ...
    function initCandidateModalLogic() {
        ...
        async function loadCand() {
            tbody.innerHTML = '<tr><td colspan="5">Tải...</td></tr>';
            ...
            data.interviewees.forEach(u => {
                const status = u.status ? '<span style="color:green">Đã nộp</span>' : 'Chưa nộp';
        ...
        tbody.addEventListener('click', async (e)=>{
            ...
            if(btn.classList.contains('save-n')) {
                await fetch(`${NGROK_BASE_URL}/manageInterviewer.php`, { method:'POST', credentials:'include', headers:{'Content-Type':'application/x-www-form-urlencoded','ngrok-skip-browser-warning':'true'}, body:new URLSearchParams({'action':'update','interview_name':currentManagingInterview,'username_to_update':u,'fullname':document.getElementById(`n-${u}`).value}) });
                alert('Đã lưu');
            }
            else if(btn.classList.contains('del-u')) {
                if(confirm('Xóa?')) {
                    await fetch(`${NGROK_BASE_URL}/manageInterviewer.php`, { method:'POST', credentials:'include', headers:{'Content-Type':'application/x-www-form-urlencoded','ngrok-skip-browser-warning':'true'}, body:new URLSearchParams({'action':'delete','interview_name':currentManagingInterview,'username_to_delete':u}) });
                    loadCand();
                }
            }
        ...
        document.getElementById('modal-add-interviewee-btn').onclick = async () => {
            ...
            const data = await res.json();
            document.getElementById('modal-status-msg').textContent = `Mã: ${data.newUser.joincode}`;
            loadCand();
        };
    }
    ...
    function initContentModalLogic() {
        ...
        window.openContentModal = async (id) => {
            ...
            json.data.forEach(q => {
                ...
                d.innerHTML = `<h4>Câu ${q.id} (Giây: <input class="tl-in" data-id="${q.id}" value="${q.timeLimit}" style="width:40px">)</h4>
                <textarea class="q-in" data-id="${q.id}">${q.question}</textarea><textarea class="c-in" data-id="${q.id}">${q.criteria}</textarea>`;
                cont.appendChild(d);
            });
        };
        ...
        document.getElementById('content-form').addEventListener('submit', async (e)=>{
            e.preventDefault();
            const list = [];
            document.querySelectorAll('.q-in').forEach(el => {
                const id = el.dataset.id;
                list.push({ id: id, question: el.value, criteria: document.querySelector(`.c-in[data-id="${id}"]`).value, timeLimit: document.querySelector(`.tl-in[data-id="${id}"]`).value });
            });
    ...
    function initGradingModalLogic() {
        ...
        window.openGradingModal = async (intId, u) => {
            ...
            json.data.forEach(q => {
                ...
                d.innerHTML = `<h4>Câu ${q.id}</h4><span>Điểm: ${q.score}</span>`;
                d.onclick = () => showDetail(q);
                listEl.appendChild(d);
            });
            if(json.data.length>0) showDetail(json.data[0]);
        };
        document.getElementById('grading-close-btn').onclick = () => modal.style.display='none';
        ...
        function showDetail(q) {
            ...
            document.getElementById('detail-history').textContent = q.history;
